# Contribute to `scaleway-csi`

`scaleway-csi` is Apache 2.0 licensed and accepts contributions via GitHub.
...
- The version of `scaleway-csi` you are using (v0.1.0, master, ...)
- Kubernetes apiserver version, flags and environments used if used with Kubernetes
- Kubelet version, flags and environments used if used with Kubernetes
- Go version
- GOOS
- GOARCH
...
- Open a pull request against `scaleway-csi` master branch that follows [PR guidelines](#pull-request-guidelines)

The maintainers of `scaleway-csi` use a "Let's Get This Merged" (LGTM) message in the pull request to note that the commits are ready to merge.
